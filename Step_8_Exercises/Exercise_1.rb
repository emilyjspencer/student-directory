...
def print_names(students) # Passing our students variable as students argument because
  students.each_with_index do |student, index| # methods don't have access to the local variables 
    puts "#{index + 1}. #{student[:name]} (#{student[:cohort]} cohort)"    # defined outside them  - but we are no longer only passing names
